...
Il costrutto `callable` visto in precedenza non è li a caso: infatti in chiron le funzioni non sono nient'altro che variabili.

In pratica, *tutte le funzioni* Chiron sono segretamente delle *lambda*.

Di conseguenza concetti come _puntatori a funzione_ non esistono
...
È possibile quindi inizializzare una funzione senza assegnarle un corpo.

[source, cpp]
----
auto foo(int a, int b); // auto gets the type and return type, but not the arguments .//
...
Bisogna fare attenzione a non confondere i blocchi di funzione con le mappe!
Infati, in caso di ambiguità il keyword auto *assumerà automaticamente che si tratti di una mappa*

[source, cpp]
----
auto foo = {}; // assumes 'foo' it's a map .//

foo = {print("Hello, World!"); // raises TypeError .//
----

=== 6.5 Argomenti delle funzioni

È inoltre possibile cambiare gli argomenti di una funzione accedendo alla proprietà **args**.
Esso non è nient'altro che una tupla.

.Assegnazione di argomenti
[source, cpp]
----
callable multiply() -> int;

multiply.args = (int a);
multiply.args:append(int b);
...
Importando 'conv' dalla standard library (_vedi sez._ 10.3) Chiron si può accedere ai classici metodi di conversione come str(), int() ma anche uno  particolare, il metodo body()

Quello che fa in metodo func è prendere in ingresso una stringa multi-linea e la valida come normale codice chiron, restituendo un oggetto body

[source, python]
----
str my_code = "result = a + b;return result;"

auto foo(int a, int b) = body(my_code);
----
